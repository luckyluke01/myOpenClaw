...
### 5月16日晚间
- git-commit-tracker (5/15 20:00 ~ 5/16 20:00): 0次提交，无数据
- 待办 12个（进行中，无变化）
- MEMORY.md 例行更新
...
## CK Query Error Report 故障记录

### 2026-05-17 21:05
- **问题**: 查询超时（120秒）
- **详情**: curl 请求到 `macs.bonree.com/OneData/clickhouse/sql/query` 执行 SQL `SELECT th, errorCodeToName(exception_code)...` 超时
- **影响**: 每日报表生成失败，未 git push
- **可能原因**: 查询数据量过大或 ClickHouse 集群响应慢
