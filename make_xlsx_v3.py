...
# First pass: collect all UIDs that need historyId
needed_uids = set()
...
# Read test case files to get historyId mapping
test_cases_dir = '/mnt/e/chromedoanload/allure-report/allure-report/data/test-cases'
historyId_map = {}

# Batch read files - only read ones we need
files_read = 0
    ...
    if os.path.exists(filepath):
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                tc = json.load(f)
                historyId_map[uid] = tc.get('historyId', '')
                files_read += 1
        except:
            historyId_map[uid] = ''
    if files_read % 2000 == 0:
        print(f"  Read {files_read} files...")

print(f"Total files read: {files_read}")

# Now build failed cases with historyId
failed_cases = []
...
<fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF4472C4"/><bgColor rgb="FF4472C4"/></patternFill></fill></fills>
...
<cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="1" borderId="0" xfId="0" applyFont="1" applyFill="1"/><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" wrapText="1"/></cellXfs>
    ...
    # Build rows - now with historyId as column 3
    rows = []
    ...
    # Column widths: A=序号(8), B=UID(20), C=HistoryId(40), D=测试名称(30), E=指标名称(45), F=函数(10), G=维度(60), H=错误维度(25), I=错误大类(18), J=错误信息(100), K=分类(15), L=状态(10)
